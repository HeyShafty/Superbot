  ...
  */
  ...
  function YouTubePlay() {
    ytdl.getInfo(args[0], (e, info) => {
      ...
      if (!client.music.hasOwnProperty(message.guild.id)) {
        client.music[message.guild.id] = {};
        client.music[message.guild.id].playing = false;
        client.music[message.guild.id].songs = [];
      }
      client.music[message.guild.id].songs.push({
        url: args[0],
        title: info.title,
      ...
      if (client.music[message.guild.id].playing === false) {
        (function play(music) { // je sais pas trop d'où vient la variable music
          ...
          client.music[message.guild.id].playing = true;
          dispatcher.on('end', () => {
            play(client.music[message.guild.id].songs.shift());
          });
          dispatcher.on('error', err => message.channel.send(`error: ${err}`).then(() => {
            play(client.music[message.guild.id].songs.shift());
          }));
        }(client.music[message.guild.id].songs.shift()));
      }
    });
  }
